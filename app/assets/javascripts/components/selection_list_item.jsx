...
App.SelectionListItemSelectInput = React.createClass({
  onChange() {
    ...
    quandon = { id: parseInt(event.target.value), group_id: groupId, isChecked: false};
    ...
    return (
      <select onChange={this.onChange}>
        {this.props.variations.map((variation) => {
          return (
            <option value={variation.id} data-group-id={variation.group_id}>{variation.name} - {App.Money.centsToFormatted(variation.amount_cents)}</option>
            );
        })}
      </select>
    );
